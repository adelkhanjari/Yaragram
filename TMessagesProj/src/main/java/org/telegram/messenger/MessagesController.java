...
public class MessagesController implements NotificationCenter.NotificationCenterDelegate
{
	...
	// Adel
	public void openByUserNameAsHidden(String username)
	{
		TLRPC.TL_contacts_resolveUsername req = new TLRPC.TL_contacts_resolveUsername();
		req.username = username;
		ConnectionsManager.getInstance().sendRequest(req, new RequestDelegate()
	...
	// Adel
	private void deleteADialog(TLRPC.TL_dialog dialog)
	{
		dialogsChannelOnly.remove(dialog); // Adel
		dialogsJustGroupsOnly.remove(dialog); // Adel
		dialogsContactOnly.remove(dialog); // Adel
						...
						statusRequest = ConnectionsManager.getInstance().sendRequest(req, new RequestDelegate()
						{
							...
							public void run(TLObject response, TLRPC.TL_error error)
							{
								if (error == null)
								{
									...
									offlineSent = Setting.getGhostMode(); // Adel
									statusSettingState = 0;
								}
								else
								{
		...
		for (int a = 0; a < dialogs.size(); a++)
		{
			...
			if (lower_id != 0 && high_id != 1)
			{
				...
				else if (lower_id > 0) // Adel
				{
					...
					if (user == null || !user.bot)
					{
						dialogsContactOnly.add(d); // Adel
					}
					else
					{
						// Adel hideBots Start
						if (user.username != null && hideChannelController.is(user.username))
						{
							try
							{
								a--;
						...
						dialogsBotOnly.add(d); // Adel
					}
				}
			}
			else if (lower_id == 0) // Adel, This is Secret Chat
			{
				dialogsContactOnly.add(d); // Adel
			}

			// Adel
			if (FavoriteController.IsFaver(d.id))
			{
	...
	// Adel
	public void processUnreadDialogs()
	{
